...
ul>li:first-child会选中ul后的第一个元素且元素是li的，但是ul后第一个是span，所以没有元素被选中       ul>li:first-of-type会选中ul后的第一个li,不计算除li外的元素（span），所以ul后第一个li被选中了       
:not()：否定伪类，将符合条件的元素从选择器中剔除       
	案例：ul>li:not(:nth-child(3))：选中ul后所有li，除了第三个元素且元素需要是li        		
...
​         此时div是一个160px的正方形,div的样式里如果添加font-size: 10px;则div会变成100px的正方形    	 rem：        
​         rem是根据根元素的字体大小进行计算        
​         案例：   
